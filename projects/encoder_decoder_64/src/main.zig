...
const Base64 = struct {
    ...
    pub fn init() Base64 {
        const upper = "ABCDEFGHIJKLMNOPKRSTUVWXYZ";
    ...
    pub fn encode(self: Base64, allocator: std.mem.Allocator, input: []const u8) ![]u8 {
        ...
        const n_out = try _calc_encode_lenght(input);
        ...
        if (count == 2) {
            out[iout] = self._char_at(buf[0] >> 2);
            out[iout + 1] = self._char_at(((buf[0] & 0x03) << 4) + (buf[1] >> 2));
    ...
    pub fn decode(self: Base64, allocator: std.mem.Allocator, input: []const u8) ![]u8 {
        ...
        const n_output = try _calc_decode_lenght(input);
...
pub fn main(init: std.process.Init) !void {
    ...
    // todo esse role para dar um HELLO WORLD
